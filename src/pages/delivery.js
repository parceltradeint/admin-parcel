...
const Delivery = () => {
  ...
  return (
    <Layout>
      <header className="-mt-6 -mx-8 bg-white shadow-sm">
        <div className=" mx-auto py-4 px-4 sm:px-6 lg:px-8  flex justify-between items-center flex-wrap sm:flex-no-wrap">
          <div className={"ml-4 "}>
              ...
              <div className=" flex rounded-md shadow-sm">
                <div className="relative flex items-stretch flex-grow focus-within:z-10">
                  <input
                    ...
                    value={searchInput}
                    className="form-input px-2 block w-full rounded-none rounded-l-md transition ease-in-out duration-150 sm:text-sm sm:leading-5"
                    placeholder={"Search"}
                  />
                </div>
                <button
...
export async function getServerSideProps({ query }) {
  ...
  return {
    props: {
      totalCount: 1, //posts.data._meta.totalCount,
      pageCount: 1, //posts.data._meta.pageCount,
      currentPage: page, //posts.data._meta.currentPage,
      perPage: 30, //posts.data._meta.perPage,
      posts: 101, //posts.data.result,
    },
  };
}
